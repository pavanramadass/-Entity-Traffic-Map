...
func TestImportDataNotEqual(t *testing.T) {
	var data Data
	data.ImportData(diffTestData)
...
func TestStoreData(t *testing.T) {
	...
	var diff = []image.Point{{X: 161, Y: 96}}
	sec := time.Now().UnixMilli()
	data.StoreData(diff)

	lastPoint := data.data[len(data.data)-1]

	if lastPoint.X != diff[0].X || lastPoint.Y != diff[0].Y || lastPoint.Timestamp != sec {
		t.Errorf("Point: %+v with Time: %d != Centroid %+v", diff[0], sec, lastPoint)
	}
}
...
func TestGetDataNoFilters(t *testing.T) {
	...
	var diffData Data
	diffData.ImportData(diffTestData)
